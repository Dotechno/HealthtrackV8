...
export default function EquipmentOwnedCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setWarranty(initialValues.warranty);
    setErrors({});
  };
  const validations = {
    datePurchased: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(new EquipmentOwned(modelFields));
          if (onSuccess) {
      ...
      {...getOverrideProps(overrides, "EquipmentOwnedCreateForm")}
      {...rest}
    >
      <TextField
